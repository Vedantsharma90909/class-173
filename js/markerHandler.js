...
AFRAME.registerComponent("markerhandler", {
  ...
  handleRatings: async function (dish) {
    var t_number
    tableNumber <= 9?(t_number = `T0${tableNumber}`):`T${tableNumber}`
    var orderSummary = await this.getOrderSummary(t_number);
    ...
    if (current_orders.length > 0 && current_orders == dish.id){
      ...
      var save_rating_btn = document.getElementById("save-rating-button")
      save_rating_btn.addEventListener("click", (e)=>{
